...
def plt_scatter(ax, xx, yy, target=None, model=None):
    ...
    if target=='PM10':
        ...
        plt.ylabel('estimated PM$_{10}$ [${\mu}g/m^{3}$]', fontsize=9)
    if target=='PM25':
        plt.xlabel('measured PM$_{2.5}$ [${\mu}g/m^{3}$]', fontsize=9)
        plt.ylabel('estimated PM$_{2.5}$ [${\mu}g/m^{3}$]', fontsize=9)
    ...
    ax.text(80, 5.5, 'N='+str(len(xx)),
            color='k', fontsize=9)
    if target=='PM10': ax.text(2, 800, 'a)', fontsize=10, weight="bold")
    if target=='PM25': ax.text(2, 800, 'c)', fontsize=10, weight="bold")
...
def plt_err(ax, xx, yy, target=None):
    ...
    if target=='PM10':
        ...
        plt.xlabel('measured PM$_{10}$ [${\mu}g/m^{3}$]', fontsize=9)
        plt.text(2, 3.2, 'b)', fontsize=10, weight="bold")
    if target=='PM25':
        plt.xlabel('measured PM$_{2.5}$ [${\mu}g/m^{3}$]', fontsize=9)
        plt.text(2, 3.2, 'd)', fontsize=10, weight="bold")
...
gs = gridspec.GridSpec(2, 3,
                       ...
                       left=.12, bottom=.1, top=.9, right=.98)
